	...
	function processNode(node: any): any {
		if (node.type === 'element') {
			...
			if (node.tagName === 'pre') {
				...
				node.properties.style += `background-color: ${theme.background} !important; color: ${theme.foreground} !important; margin: 0; padding: 1rem; font-family: monospace; white-space: pre-wrap; word-wrap: break-word;`;
			}
			...
			if (node.properties && node.properties.className) {
				...
				for (const cls of classes) {
					if (cls in theme.colors) {
						color = (theme.colors as any)[cls];
						break;
					}
				}
